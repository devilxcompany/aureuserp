...
it('returns the latest version tag from a GitHub release', function () {
    Http::fake([
        'https://api.github.com/repos/devilxcompany/aureuserp/releases/latest' => Http::response([
            'tag_name' => 'v1.2.3',
            'name'     => 'Version 1.2.3',
        ], 200),
    ]);
    ...
    expect($github->latestVersion('devilxcompany', 'aureuserp'))->toBe('v1.2.3');
});

it('returns null when the releases endpoint returns 404', function () {
    Http::fake([
        'https://api.github.com/repos/devilxcompany/nonexistent/releases/latest' => Http::response([], 404),
    ]);

    $github = new GitHub;

    expect($github->latestVersion('devilxcompany', 'nonexistent'))->toBeNull();
});
...
it('returns null when a connection exception occurs', function () {
    ...
    expect($github->latestVersion('devilxcompany', 'aureuserp'))->toBeNull();
});
...
it('returns the latest release data', function () {
    ...
    $github = new GitHub;
    $release = $github->latestRelease('devilxcompany', 'aureuserp');
...
it('returns a list of releases', function () {
    Http::fake([
        'https://api.github.com/repos/devilxcompany/aureuserp/releases*' => Http::response([
    ...
    $github = new GitHub;
    $releases = $github->releases('devilxcompany', 'aureuserp');
...
it('returns an empty array when releases endpoint fails', function () {
    ...
    expect($github->releases('devilxcompany', 'aureuserp'))->toBeEmpty();
});

it('returns repository metadata', function () {
    Http::fake([
        'https://api.github.com/repos/devilxcompany/aureuserp' => Http::response([
            'full_name'        => 'devilxcompany/aureuserp',
    ...
    $github = new GitHub;
    $repo = $github->repository('devilxcompany', 'aureuserp');

    expect($repo)
        ->toBeArray()
        ->and($repo['full_name'])->toBe('devilxcompany/aureuserp');
});
...
it('sends the GitHub API version header in every request', function () {
    ...
    $github = new GitHub;
    $github->latestRelease('devilxcompany', 'aureuserp');
